...
[Zebra](/zcash-tech/zebra-full-node) è il nodo della Zcash Foundation, scritto in Rust, ed è quello su cui si basano ora la maggior parte delle guide. [Il libro di Zebra](https://zebra.zfnd.org/) spiega come installarlo ed eseguirlo, mentre il [repository](https://github.com/ZcashFoundation/zebra) è dove avviene lo sviluppo.

[Zakura](/zcash-tech/zakura-node) è un nodo più recente, descritto dai suoi autori come un "nodo completo Zcash compatibile con il consenso, realizzato per scalare", con sincronizzazione più rapida, pruning dei blocchi e una modalità di compatibilità con zcashd. È guidato da Sean Bowe, cofondatore di Zcash, e Dev Ojha. È open source con licenza Apache 2.0 su [zakura-core/zakura](https://github.com/zakura-core/zakura).

ZecHub ha una pagina [Nodi completi](/zcash-tech/full-nodes) che tratta i compromessi tra le due soluzioni.

## Il wallet per nodo completo

zcashd includeva un wallet nel nodo. Quel wallet non c'è più e [Zallet](https://github.com/zcash/zallet) lo sostituisce. Il libro di Zallet lo descrive come "un wallet Zcash per nodo completo scritto in Rust" e "realizzato come sostituto del wallet zcashd".

Leggi l'avviso di sicurezza prima di farvi affidamento. Zallet è in beta, "non è stato sottoposto a una revisione completa", potrebbero verificarsi modifiche incompatibili "in qualsiasi momento, che richiederanno di eliminare e ricreare il tuo wallet Zallet", e non tutti i metodi RPC di zcashd sono stati ancora portati.
...
La documentazione del [Protocollo per client leggeri](https://zcash.readthedocs.io/en/latest/lightwalletd/index.html) tratta il protocollo stesso. La pagina [Nodi Lightwallet](/zcash-tech/lightwallet-nodes) spiega cosa questi server possono e non possono vedere di un utente; vale la pena capirlo prima di sceglierne uno.
...
| Crate | A cosa serve |
|:--|:--|
| zcash_client_backend | "API per creare client leggeri Zcash schermati", inclusi sincronizzazione e costruzione delle transazioni |
...
| pczt | "Strumenti per lavorare con transazioni Zcash create parzialmente", usati per la firma hardware e multi-dispositivo |
| zip321 | URI di richieste di pagamento, come specificato in ZIP 321 |

Per il mobile, l'[SDK Android](https://github.com/zcash/zcash-android-wallet-sdk) e l'[SDK iOS](https://github.com/zcash/zcash-swift-wallet-sdk) includono tali librerie. Il repository iOS si chiamava in precedenza ZcashLightClientKit, quindi i link e gli articoli meno recenti usano quel nome.

## Specifica e crittografia

La [specifica del protocollo](https://zips.z.cash/protocol/protocol.pdf) è l'autorità di riferimento sul funzionamento di Zcash, comprese le [codifiche di indirizzi e chiavi](https://zips.z.cash/protocol/protocol.pdf#5.6%20Encodings%20of%20Addresses%20and%20Keys).

Gli [ZIP](https://zips.z.cash) sono il luogo in cui le modifiche vengono proposte e specificate, e l'indice mostra quali sono bozze e quali definitivi. Le modifiche al consenso vengono distribuite negli aggiornamenti di rete, che ZecHub monitora nella pagina [Aggiornamenti di rete](/start-here/network-upgrades).
...
Testnet è una catena separata con monete prive di valore, chiamate TAZ. Sia Zebra sia Zakura possono funzionare su di essa, e la [guida alla testnet](https://zcash.readthedocs.io/en/latest/rtd_pages/testnet_guide.html) tratta la configurazione del nodo.
...
Ottenere TAZ è la parte complicata. I faucet pubblici appaiono e scompaiono, e quelli collegati dalla documentazione meno recente non rispondevano quando questa pagina è stata scritta. Il percorso affidabile è chiedere nel Discord R&D di Zcash, che è ciò che suggerisce la stessa documentazione di Zcash.

## Documentazione generale

La [Documentazione Zcash](https://zcash.readthedocs.io/en/latest/) è ancora la singola fonte più ampia e tratta concetti del protocollo, integrazione e mining. Leggila con una certa attenzione. È versionata rispetto a zcashd, quindi alcune parti descrivono un nodo che non è più operativo, mentre le sezioni sul protocollo e sui client leggeri rimangono utili. Vale la pena leggere [Il modello di minaccia dell'app wallet Zcash](https://zcash.readthedocs.io/en/latest/rtd_pages/wallet_threat_model.html) ospitato lì prima di progettare qualsiasi cosa che tocchi la privacy degli utenti.
...
[Arti](https://docs.rs/arti/latest/arti/) è l'implementazione Rust di Tor, usata da zcash_client_backend per instradare il traffico dei wallet. [Tailscale](https://github.com/tailscale/tailscale) viene menzionato per connettersi a un nodo eseguito in proprio. [warp2](https://github.com/hhanh00/warp2) è un'implementazione di sincronizzazione rapida di Hanh, sebbene non sia stata aggiornata dal 2023.
...
Se hai una risorsa da aggiungere, oppure noti qualcosa qui che non è più aggiornato, apri una issue o una pull request. I team non sempre hanno la capacità di mantenere tutto aggiornato, e segnalare ciò che hai incontrato aiuta a orientare le guide.
