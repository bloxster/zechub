...
>
> Ogni passaggio qui sotto dipende da zcashd, che ha raggiunto l'arresto automatico di Fine Supporto il 18 luglio 2026. I sette script distribuiti insieme a questa pagina lo utilizzano tramite `zcash-cli`, quindi oggi nessuno di essi può raggiungere un nodo in esecuzione.
>
> Questi script non possono essere portati meccanicamente. Si basano sulle RPC per transazioni raw e wallet (`createrawtransaction`, `signrawtransaction`, `createmultisig`, `dumpprivkey`) che zcashd ha deprecato prima dell'arresto; Zallet le sostituisce con nuovi metodi che operano su PCZT anziché su hex di transazioni raw, ed è ancora in beta con molti metodi zcashd non ancora portati.
>
...
>
> Questa pagina viene conservata come testimonianza storica del flusso di lavoro multisig trasparente.
...
* Se utilizzi zcashd, puoi anche creare un UA e usare il tuo ricevente trasparente. Quindi usa `getPubkey.sh` per estrarre la tua chiave pubblica.
...
ma nella maggior parte degli esempi reali la firma verrà effettuata da persone in tutto il mondo, quindi ciascuno dei partecipanti necessari dovrà firmare,
quindi rimandare l'output aggiornato in "hex" della raxTX che gli altri utilizzeranno per firmare e completare la procedura di firma.
...
Per firmare questa tx, devono firmarla almeno 2 delle tre chiavi private. Se la chiave pubblica fornita è stata esportata usando un indirizzo T da zcashd, puoi ottenere la chiave privata del tuo indirizzo T con: 
